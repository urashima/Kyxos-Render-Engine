export const PHASE_04_PBR_TEMPORAL_OUTPUT_WGSL = `const PBR_PI: f32 = 3.141592653589793;
...
@group(1) @binding(0) var diffuseIrradianceTexture: texture_cube<f32>;
@group(1) @binding(1) var specularPrefilterTexture: texture_cube<f32>;
@group(1) @binding(2) var environmentCubeSampler: sampler;
@group(1) @binding(3) var brdfLutTexture: texture_2d<f32>;
@group(1) @binding(4) var brdfLutSampler: sampler;
...
fn pbrRotateEnvironmentDirection(direction: vec3f) -> vec3f {
  let rotation = object.occlusionEnvironmentRotations.zw;
  return vec3f(
    rotation.x * direction.x - rotation.y * direction.z,
    direction.y,
...
fn pbrNeutralToneMap(inputColor: vec3f) -> vec3f {
...
fn pbrLinearChannelToSrgb(channel: f32) -> f32 {
...
fn pbrLinearToSrgb(color: vec3f) -> vec3f {
  return vec3f(
    pbrLinearChannelToSrgb(color.r),
    pbrLinearChannelToSrgb(color.g),
    pbrLinearChannelToSrgb(color.b),
  );
}

fn pbrApplyOutputTransform(linearHdr: vec3f) -> vec3f {
  let exposed = max(linearHdr, vec3f(0.0)) * object.environmentControls.z;
  let toneMapped = select(
    clamp(exposed, vec3f(0.0), vec3f(1.0)),
    pbrNeutralToneMap(exposed),
    object.environmentControls.w >= 0.5,
  );
  return pbrLinearToSrgb(clamp(toneMapped, vec3f(0.0), vec3f(1.0)));
}
...
fn shadePbr(input: VertexOutput, frontFacing: bool) -> PbrShadingResult {
  ...
  let metallic = material.x * metallicRoughnessSample.b;
  let perceptualRoughness = material.y * metallicRoughnessSample.g;
  ...
  let diffuseDirection = pbrRotateEnvironmentDirection(normal);
  let reflectionDirection = pbrRotateEnvironmentDirection(
    reflect(-viewDirection, normal),
  );
  let diffuseIrradiance = textureSampleLevel(
    diffuseIrradianceTexture,
    environmentCubeSampler,
    diffuseDirection,
    0.0,
  ).rgb;
  let prefilteredSpecular = textureSampleLevel(
    specularPrefilterTexture,
    environmentCubeSampler,
    reflectionDirection,
    perceptualRoughness * object.environmentControls.y,
  ).rgb;
  let brdfLut = textureSampleLevel(
    brdfLutTexture,
    brdfLutSampler,
    vec2f(nDotV, perceptualRoughness),
    0.0,
  ).rg;
  let diffuseColor = baseColor.rgb * (1.0 - metallic);
  let f0 = mix(vec3f(0.04), baseColor.rgb, vec3f(metallic));
  let indirectDiffuse = diffuseIrradiance * diffuseColor / PBR_PI;
  let indirectSpecular = prefilteredSpecular * (f0 * brdfLut.x + vec3f(brdfLut.y));
  ...
  let indirectRadiance =
    (indirectDiffuse + indirectSpecular) *
    ambientOcclusion *
    object.environmentControls.x;
